...
/** 商品复制规则表,针对字符串过滤,数字最大最小等*/
import {BaseSysGroup} from "@/TsModel/Alien/Entity/Tabls/BaseSysGroup";
export interface t_wmt_fc_fdmv_rule extends BaseSysGroup {
		/** 规则名称自动保存时将以此为key 来去重*/
		name?: string | null;
		/** 店铺类型*/
		shop_type: ShopType;
		/** 针对事件型.如果这个不为空,当么这条规则只支持事件触发*/
		for_evnt?: RuleForEvent | null;
		/** 针对事件关键词*/
		evnt_words?: string | null;
		/** 关键词用正则匹配*/
		evnt_reg?: boolean | null;
		/** 当正则匹配成功,时, 将匹配成功的第一个项设置到 this.r_word_old*/
		evnt_set_rword?: boolean | null;
		/** 如果 evnt_set_rword 为true 代表要设置的字段类型 如果为空将默认设置   this.r_word_old*/
